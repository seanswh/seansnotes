...
半角方向和镜面方向。如果光泽度是0，那么它和Lambertian（漫反射）的情况就很像了，除了这里使用的是半角，而不是漫反射使用的光源方向。
   最后，一定要注意 both light and view directions point **TO** the light or viewer (and away from the surface). The signs on some formulas, such as for the reflection vector R, would be different if L pointed toward the surface instead.
