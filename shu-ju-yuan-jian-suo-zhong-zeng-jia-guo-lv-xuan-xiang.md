...
2. 每一次过滤检索，都要递归遍历所有的Node，这个根本没必要！只要检查当前节点的所有下一级子节点就行了！，比如第一次只需要判断mdfs根节点和所有的二级节点。如果用户点击了”数据源检索“二级节点中的“EC细网格”节点以后，就只判断”EC细网格“节点下面的子节点在不在dic里面key=“EC细网格"下面的节点即可，没必要递归。
3. FindSearchNode函数：
